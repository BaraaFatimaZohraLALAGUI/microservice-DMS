...
public class DocumentService {
    ...
    public DocumentViewDto createDocument(DocumentCreateRequestDto dto, String ownerUserId) {
        // FIX: Use the 'dto' instance to access its fields, not the class name statically
    ...
    public DocumentViewDto getDocumentById(Long id, String userId, List<String> userRoles) {
        ...
        // Authorization check: Admin can see everything, User can only see docs in their departments
    ...
    public Page<DocumentViewDto> findDocumentsForUser(String userId, Pageable pageable) {
        ...
        log.debug("Found {} documents for user {} in departments {}", documentPage.getTotalElements(), userId, departmentIds);
    ...
    public DocumentViewDto updateTranslatedTitle(Long documentId, String titleEs) {
        ...
        Document document = documentRepository.findById(documentId) // Fetch without details is fine here
             .orElseThrow(() -> new ResourceNotFoundException("Document not found with id: " + documentId));

        document.setTitleEs(titleEs);
        Document updatedDocument = documentRepository.save(document);

        // Need to fetch details again for the response DTO
        // Ensure your repository method `findByIdWithDetails` exists and is correct
         Document detailedDocument = documentRepository.findByIdWithDetails(updatedDocument.getId())
                 .orElse(updatedDocument); // Fallback in case findByIdWithDetails fails (shouldn't ideally)

        log.info("Successfully updated Spanish title for document ID: {}", documentId);
        return mapToViewDto(detailedDocument);
    }
    ...
    public void deleteDocument(Long id) {
         ...
         // kafkaProducerService.sendDocumentDeletedEvent(document.getS3FileKey()); // Example
    ...
    private DocumentViewDto mapToViewDto(Document document) {
        // Handles potential nulls if category/department weren't fetched properly (shouldn't happen with JOIN FETCH)
        ...
        // Ensure DocumentViewDto has a builder method or a constructor matching these fields
